...
test.describe('Tabs', () => {
  ...
  test('Click on a tab, expect aria-selected attribute to be "true".', async ({ page }) => {
    ...
    let thirdTabButton = await page.$eval('button#tab-3', (button) => button.getAttribute('aria-selected'))
  ...
  test('Click on a tab, expect hidden attribute of the panel to be null.', async ({ page }) => {
    await page.click('button#tab-3')
    let thirdTabPanel = await page.$eval('#tab-panel-3', (panel) => panel.getAttribute('hidden'))
