...
test.describe('Modal', () => {
  ...
  test('Click on a dialog trigger button, expect the modal dialog is visible.', async ({ page }) => {
    ...
    let display = await page.$eval('#demo-1', (dialog) => window.getComputedStyle(dialog).display)
  ...
  test('On a 1025px window width page, click on a modal button working only for <= 1024px window width pages, expect the content is still visible.', async ({
    page,
  }) => {
